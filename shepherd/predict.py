...
def predict(args):
    ...
    # Get dataset
    spl_infile = project_config.PROJECT_DIR / 'patients' / hparams['spl']
    logger.info(f'Loading shortest path length file: {spl_infile}')
    spl = np.load(spl_infile)
    logger.info(f"Done loading shortest path length file: {spl_infile}")

    # Load SPL indexing dict
    spl_index_infile = project_config.PROJECT_DIR / 'patients' / hparams['spl_index']
    if (spl_index_infile).exists():
        logger.info(f"Loading SPL indexing dict file: {spl_index_infile}")
        with open(spl_index_infile, "rb") as input_file:
            spl_indexing_dict = pickle.load(input_file)
        logger.info(f"Done loading SPL indexing dict file: {spl_index_infile}")
    else:
        spl_indexing_dict = None

    # Load patient datasets
    dataset_infile = project_config.PROJECT_DIR / 'patients' / hparams['test_data']
    logger.info(f"Loading patient datasets from {dataset_infile}")
    dataset = PatientDataset(dataset_infile, time=hparams['time'])
    logger.info(f"Done loading patient datasets from {dataset_infile}")

    logger.info(f'There are {len(dataset)} patients in the test dataset')
    hparams.update({'inference_batch_size': len(dataset)})
    logger.info(f"batch size: {hparams['inference_batch_size']}")

    # Get node idx to shortest path length dict for gene-protein nodes
